...
export class Bundling {
  ...
  public static parcel(options: ParcelOptions): lambda.AssetCode {
    ...
    const image = cdk.BundlingDockerImage.fromAsset(
      path.join(__dirname, "../parcel"),
      {
        buildArgs: {
          ...(options.buildArgs ?? {}),
    ...
    const containerEntryPath = path.join(
      cdk.AssetStaging.BUNDLING_INPUT_DIR,
      path.relative(projectRoot, path.resolve(options.entry)),
    );
    const parcelCommand = [
      "$(node -p \"require.resolve('parcel')\")", // Parcel is not globally installed, find its "bin"
      "build",
      containerEntryPath.replace(/\\/g, "/"), // Always use POSIX paths in the container
      "--target",
      "cdk-lambda",
      "--no-autoinstall",
      "--no-scope-hoist",
    ...
    if (dependencies) {
      ...
      depsCommand = chain([
        ...
        lockfile
          ? `cp ${cdk.AssetStaging.BUNDLING_INPUT_DIR}/${lockfile} ${cdk.AssetStaging.BUNDLING_OUTPUT_DIR}/${lockfile}`
          : "",
